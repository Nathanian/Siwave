package com.bro.siwave.ui.preset;

import android.os.Bundle;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;
import androidx.fragment.app.Fragment;
import androidx.recyclerview.widget.LinearLayoutManager;
import androidx.recyclerview.widget.RecyclerView;

import com.bro.siwave.R;
import com.bro.siwave.TrainingFragment;
import com.bro.siwave.preset.PresetLoader;
import com.bro.siwave.preset.PresetProgram;
import com.bro.siwave.session.TrainingSessionManager;

import java.util.List;

public class PresetFragment extends Fragment {

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_presets, container, false);

        RecyclerView recyclerView = view.findViewById(R.id.recyclerViewPresets);
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));

        List<PresetProgram> presets = PresetLoader.loadPresets(requireContext());
        PresetAdapter adapter = new PresetAdapter(presets, preset -> {
            TrainingSessionManager.getInstance().startVordefiniert(preset);
            requireActivity().getSupportFragmentManager()
                    .beginTransaction()
                    .replace(R.id.fragment_container, new TrainingFragment()) // Falls du direkt zurück willst
                    .commit();
        });

        recyclerView.setAdapter(adapter);
        return view;
    }
}
